...
void SimpleShadowmapRender::copyNoise()
{
  ...
  // commands
  etna::set_state(m_cmdBufferAux, noiseTexture.get(), vk::PipelineStageFlagBits2::eTransfer, vk::AccessFlagBits2::eTransferWrite, vk::ImageLayout::eTransferDstOptimal, vk::ImageAspectFlagBits::eColor);
  ...
  etna::set_state(m_cmdBufferAux, noiseTexture.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
  etna::flush_barriers(m_cmdBufferAux);
...
void SimpleShadowmapRender::loadShaders()
{
  ...
  etna::create_program("simple_deferred", { VK_GRAPHICS_BASIC_ROOT "/resources/shaders/simple_quad.vert.spv", VK_GRAPHICS_BASIC_ROOT "/resources/shaders/simple_deferred.frag.spv" });
  etna::create_program("simple_ssao", { VK_GRAPHICS_BASIC_ROOT "/resources/shaders/simple_quad.vert.spv", VK_GRAPHICS_BASIC_ROOT "/resources/shaders/simple_ssao.frag.spv" });
...
void SimpleShadowmapRender::DrawSceneCmd(VkCommandBuffer a_cmdBuff, const float4x4 &a_wvp)
{
  ...
  for (uint32_t i = 0; i < m_pScnMgr->InstancesNum(); ++i)
  {
    ...
    vkCmdPushConstants(a_cmdBuff, m_geometryPipeline.getVkPipelineLayout(), stageFlags, 0, sizeof(pushConst2M), &pushConst2M);
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    ...
    etna::RenderTargetState renderTargets(a_cmdBuff, { m_width, m_height }, { { gbuffer.albedo, gbuffer.normals, gbuffer.positions } }, mainViewDepth);

    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_geometryPipeline.getVkPipeline());
    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_geometryPipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);
  ...
  if (ssaoEnabled)
  {
    etna::set_state(a_cmdBuff, noiseTexture.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
    etna::set_state(a_cmdBuff, gbuffer.normals.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
    etna::set_state(a_cmdBuff, gbuffer.positions.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
    ...
    //
    {
      ...
      auto set = etna::create_descriptor_set(simpleSsaoInfo.getDescriptorLayoutId(0), a_cmdBuff, { etna::Binding{ 0, kernelBuffer.genBinding() }, etna::Binding{ 1, constants.genBinding() }, etna::Binding{ 2, noiseTexture.genBinding(noiseTextureSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) }, etna::Binding{ 3, gbuffer.normals.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) }, etna::Binding{ 4, gbuffer.positions.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) } });
      ...
      vkCmdPushConstants(a_cmdBuff, m_ssaoPipeline.getVkPipelineLayout(), VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(pushConstDeferred), &pushConstDeferred);

      vkCmdDraw(a_cmdBuff, 3, 1, 0, 0);
    }

    etna::set_state(a_cmdBuff, ssaoRawImage.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
    etna::set_state(a_cmdBuff, ssaoBlurredImage.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderWrite, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
    ...
    // Apply horizontal gaussian filter in compute shader
    {
      ...
      vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_computePipelineHorizontal.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);

      vkCmdPushConstants(a_cmdBuff, m_computePipelineHorizontal.getVkPipelineLayout(), VK_SHADER_STAGE_COMPUTE_BIT, 0, m_coeffs.size() * sizeof(float), m_coeffs.data());

      vkCmdDispatch(a_cmdBuff, 1024 / 32 + 1, 1024, 1);
    }

    etna::set_state(a_cmdBuff, ssaoRawImage.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderWrite, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
    etna::set_state(a_cmdBuff, ssaoBlurredImage.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
    ...
    // Apply vertical gaussian filter in compute shader
    {
      ...
      vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_computePipelineVertical.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);

      vkCmdPushConstants(a_cmdBuff, m_computePipelineVertical.getVkPipelineLayout(), VK_SHADER_STAGE_COMPUTE_BIT, 0, m_coeffs.size() * sizeof(float), m_coeffs.data());

      vkCmdDispatch(a_cmdBuff, 1024, 1024 / 32 + 1, 1);
    }
  }

  etna::set_state(a_cmdBuff, shadowMap.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eDepth);
  etna::set_state(a_cmdBuff, mainViewDepth.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eDepth);
  etna::set_state(a_cmdBuff, gbuffer.albedo.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
  etna::set_state(a_cmdBuff, gbuffer.normals.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
  etna::set_state(a_cmdBuff, gbuffer.positions.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
  etna::set_state(a_cmdBuff, ssaoRawImage.get(), vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eShaderReadOnlyOptimal, vk::ImageAspectFlagBits::eColor);
  etna::flush_barriers(a_cmdBuff);
  ...
  //
  {
    ...
    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_shadingPipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0, VK_NULL_HANDLE);

    vkCmdPushConstants(a_cmdBuff, m_shadingPipeline.getVkPipelineLayout(), VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(pushConstDeferred), &pushConstDeferred);
